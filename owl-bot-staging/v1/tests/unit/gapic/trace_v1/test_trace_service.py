...
def test_list_traces(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.list_traces),
            '__call__') as call:
...
def test_list_traces_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, trace.Trace)
                   for i in results)
...
async def test_list_traces_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_traces(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_get_trace(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_trace),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_patch_traces(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.patch_traces),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_list_traces_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        pb_return_value = trace.ListTracesResponse.pb(return_value)
...
def test_list_traces_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.TraceServiceRestInterceptor, "post_list_traces") as post, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "pre_list_traces") as pre:
...
def test_get_trace_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = trace.Trace.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_get_trace_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "post_get_trace") as post, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "pre_get_trace") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_patch_traces_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "pre_patch_traces") as pre:
        pre.assert_not_called()
