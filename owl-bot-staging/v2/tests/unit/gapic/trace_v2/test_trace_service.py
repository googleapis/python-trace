...
@pytest.mark.parametrize("client_class", [
    TraceServiceClient, TraceServiceAsyncClient
])
@mock.patch.object(TraceServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(TraceServiceClient))
@mock.patch.object(TraceServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(TraceServiceAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (TraceServiceClient, transports.TraceServiceGrpcTransport, "grpc", grpc_helpers),
    (TraceServiceAsyncClient, transports.TraceServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_batch_write_spans_rest(request_type):
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_batch_write_spans_rest_required_fields(request_type=tracing.BatchWriteSpansRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).batch_write_spans._get_unset_required_fields(jsonified_request)
...
def test_batch_write_spans_rest_unset_required_fields():
    transport = transports.TraceServiceRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.batch_write_spans._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("name", "spans", )))


@pytest.mark.parametrize("null_interceptor", [True, False])
def test_batch_write_spans_rest_interceptors(null_interceptor):
    transport = transports.TraceServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.TraceServiceRestInterceptor(),
        )
    client = TraceServiceClient(transport=transport)
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "pre_batch_write_spans") as pre:
        pre.assert_not_called()
        pb_message = tracing.BatchWriteSpansRequest.pb(tracing.BatchWriteSpansRequest())
        ...
        client.batch_write_spans(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()


def test_batch_write_spans_rest_bad_request(transport: str = 'rest', request_type=tracing.BatchWriteSpansRequest):
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_batch_write_spans_rest_flattened():
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v2/{name=projects/*}/traces:batchWrite" % client.transport._host, args[1])


def test_batch_write_spans_rest_flattened_error(transport: str = 'rest'):
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_batch_write_spans_rest_error():
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport='rest'
    )
...
def test_create_span_rest(request_type):
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    # send a request that will satisfy transcoding
    request_init = {'name': 'projects/sample1/traces/sample2/spans/sample3'}
    request = request_type(**request_init)

    # Mock the http request call within the method and fake a response.
    with mock.patch.object(type(client.transport._session), 'request') as req:
        # Designate an appropriate value for the returned response.
        return_value = trace.Span(
              name='name_value',
              span_id='span_id_value',
              parent_span_id='parent_span_id_value',
              span_kind=trace.Span.SpanKind.INTERNAL,
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = trace.Span.pb(return_value)
...
def test_create_span_rest_required_fields(request_type=trace.Span):
    transport_class = transports.TraceServiceRestTransport

    request_init = {}
    request_init["name"] = ""
    request_init["span_id"] = ""
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(
        pb_request,
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_span._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)

    # verify required fields with default values are now present

    jsonified_request["name"] = 'name_value'
    jsonified_request["spanId"] = 'span_id_value'
    ...
    assert "name" in jsonified_request
    assert jsonified_request["name"] == 'name_value'
    assert "spanId" in jsonified_request
    assert jsonified_request["spanId"] == 'span_id_value'
...
def test_create_span_rest_unset_required_fields():
    transport = transports.TraceServiceRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.create_span._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("name", "spanId", "displayName", "startTime", "endTime", )))


@pytest.mark.parametrize("null_interceptor", [True, False])
def test_create_span_rest_interceptors(null_interceptor):
    transport = transports.TraceServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.TraceServiceRestInterceptor(),
        )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "post_create_span") as post, \
         mock.patch.object(transports.TraceServiceRestInterceptor, "pre_create_span") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = trace.Span.pb(trace.Span())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = trace.Span.to_json(trace.Span())
        ...
        pre.return_value = request, metadata
        post.return_value = trace.Span()

        client.create_span(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_create_span_rest_bad_request(transport: str = 'rest', request_type=trace.Span):
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_span_rest_error():
    client = TraceServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport='rest'
    )
...
def test_trace_service_client_transport_session_collision(transport_name):
    ...
    session1 = client1.transport.batch_write_spans._session
    session2 = client2.transport.batch_write_spans._session
    assert session1 != session2
    session1 = client1.transport.create_span._session
    session2 = client2.transport.create_span._session
    assert session1 != session2
